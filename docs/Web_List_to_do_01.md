...
| E1-05-FOLDER-TREE | E1 | Folder/Tree display bằng relational | Cấu trúc cha–con `knowledge_documents` | Dùng `parent_id` self-ref trong `knowledge_documents`; render hierarchy text (Vue Flow chỉ nếu built-in); CRUD folder qua Directus API, không tạo collection mới. | TODO |
