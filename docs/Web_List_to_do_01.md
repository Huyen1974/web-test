...
| 0036-KNOWLEDGE-CASE-ORGANIZATION | B | Thiết kế cách tổ chức Cases/tri thức theo Zone/Sub-zone | Giúp cổng tri thức dễ tìm, dễ thay đổi & quản lý khối lượng tri thức lớn | (1) Xác định taxonomy: Category (Lĩnh vực) → Zone (Khu vực) → Topic (Chủ đề) → Case/Document; (2) Map taxonomy này vào schema Agent Data; (3) Xác định quy tắc gán Zone/Sub-zone cho tài liệu mới; (4) Nuxt dùng taxonomy này để sinh menu tự động | DONE – Taxonomy 4-level design + TS scaffold; evidence: reports/0036a_taxonomy_design.md, reports/0036b_taxonomy_senior_review_and_merge.md, PR #97 (CI green). |
| 0037-BLUEPRINT-PILOT-1 | B | Tạo & duyệt 1–2 Blueprint/tài liệu thí điểm | Kiểm chứng trục Directus (phê duyệt) ↔ Nuxt (hiển thị) & Agent Data (search/log) hoạt động ổn. | (1) Chọn 1–2 nhóm tri thức hẹp (vd: 1 quy trình nội bộ + 1 bộ biểu mẫu); (2) AI soạn Blueprint/tài liệu theo Template V1 và taxonomy Zone/Sub-zone; (3) User đọc/xem thử trên Nuxt; phê duyệt chính thức được thực hiện trên Directus (Content Versioning/Directus Admin UI); (4) Ghi lại vấn đề/góp ý để tinh chỉnh Template, UI & quy trình | DONE – Knowledge Taxonomy UI (menu + điều hướng) đã tích hợp; evidence: reports/0037_taxonomy_ui_implementation.md, reports/0037b_taxonomy_ui_senior_review_and_merge.md, PR #99 (CI xanh: Nuxt build, Pass Gate, Quality Gate, E2E Smoke Test, Terraform Deploy). |

Giai đoạn C – Lark ↔ Directus (+ Agent Data) (Metadata & Blueprint từ Lark)

### Ghi chú Pivot Giai đoạn (C/D → E) – 2025-12-03

- **Tạm hoãn Phase C (Lark Sync)** cho tới khi Content Engine ổn định.
- **Tạm hoãn Phase D (Sandbox Automation)**, chuyển sang giai đoạn sau.
- **Ưu tiên Phase E (CONTENT ENGINE & VERSIONING)**:
  - Directus đóng vai trò trung tâm (Content Engine) để quản lý vòng đời nội dung: Draft → Review → Publish.
  - Internal Agents/Thư ký ảo chỉ soạn nháp (Draft) và đẩy vào Directus; con người review/bấm Publish qua Directus hoặc Nuxt (read-only).
- Nguyên tắc: **“Xây nhà xong mới dọn đồ vào”** – Hoàn thiện hệ thống quản trị nội dung trước khi nhập dữ liệu từ Lark hoặc nguồn cũ.
...
| ID | Giai đoạn | Tên Task | Mục tiêu | Nội dung chính | Trạng thái |
| --- | --- | --- | --- | --- | --- |
| 0047-VERSIONING-STRATEGY | E | Thiết kế chiến lược Versioning & Diff cho tài liệu tri thức & schema | Chốt cứng hai kênh versioning & duyệt: (1) Tài liệu tri thức dùng Content Versioning của Directus, (2) Schema kỹ thuật dùng Directus/Git. | (1) Quy định: Tài liệu tri thức sử dụng Content Versioning của Directus làm kênh versioning & phê duyệt chính; Nuxt chỉ hiển thị các phiên bản đã publish (read-only) và nếu cần Diff thì chỉ là UI xem, không là nơi quyết định duyệt; (2) Schema kỹ thuật dùng Content Versioning của Directus hoặc Git PR + snapshot.yaml; (3) Không tự code hệ thống versioning mới; (4) Lưu chiến lược thành luật hệ thống cho mọi Agent; (5) Cấu hình giới hạn lưu trữ: Tối đa 10 Revisions hoặc 7 ngày. Bật Auto-purge để tránh phình Database (Tuân thủ Điều 20). | DONE – Merge PR #106 qua CLI.CODEX.RESCUE-0047B (auto_merge_pr.sh mới, CI xanh). Bằng chứng: reports/0047a_versioning_design.md, reports/0047a_versioning_design_exec_CLAUDE.md, reports/0047b_merge_rescue_codex.md |
| 0047C-KNOWLEDGE-SCHEMA-MIGRATION | E | Migration schema Knowledge + workflow versioning cho Directus TEST | Tạo collection `knowledge_documents` và 14 trường workflow/versioning (workflow_status, version_group_id, version_number, is_current_version, previous_version_id, reviewed_by/at, approved_by/at, publisher_id, rejection_reason, purge_after, parent_document_id, child_order). Auth dùng /auth/login với SECRET Manager (email/password). | MIGRATION DONE (GREEN) – Đã chạy scripts/0047c_migration_knowledge.ts qua Directus TEST bằng email/password từ Secret Manager; xác nhận 14 trường qua API; sẵn sàng cho Nuxt read-only. Bằng chứng: reports/0047c_auth_secrets_setup.md, reports/0047c_migration_dry_run.md, reports/0047c_migration_execution.md, reports/0047d_finalization.md |
| 0047F-NUXT-INTEGRATION | E | Tích hợp 0047C workflow fields vào Nuxt frontend (READ-ONLY) | Cập nhật view-model types, composables và UI pages để hiển thị các trường workflow/versioning mới từ schema 0047C. Tuân thủ nguyên tắc READ-ONLY: không thêm chức năng approve/reject trên Nuxt. | DONE (GREEN) – Đã cập nhật KnowledgeCard/KnowledgeListEntry interfaces với 14 trường mới; useKnowledge composable query + map các trường mới và apply is_current_version filter; knowledge pages hiển thị workflow_status badges và version_number; build + lint pass; commit local trên branch feat/0047f-nuxt-integration. Bằng chứng: reports/0047f_nuxt_integration.md, commit 2308cd9c |
| 0048-WORKFLOW-ROLES-AGENT-USER | E | Định nghĩa quyền & workflow chỉnh sửa tài liệu | Phân biệt rõ Agent soạn nháp và User phê duyệt cuối + thêm RBAC cứng trong Directus | (1) Thiết kế Role: Agent chỉ được tạo/sửa Draft; User (Editor/Approver) mới được Publish; (2) Với Directus: cấu hình RBAC cứng theo nguyên tắc DENY cho Agent trên mọi bảng thuộc Core Zone – không Insert/Update/Delete, không ALTER, không DELETE/DROP; Agent chỉ được READ và thao tác trên Growth Zone/bảng log/graph; (3) Với Schema: chỉ Admin duyệt qua Directus Versioning; (4) Với Agent Data: chỉ các bản ghi đã ở trạng thái approved (theo workflow nội bộ) mới được phép index vào Qdrant/engine search và được dùng làm context cho Agent; việc hiển thị ra Nuxt vẫn phải dựa trên trạng thái & cờ user_visible trong Directus (Published + user_visible = true). Agent Data KHÔNG thêm một tầng duyệt riêng cho hiển thị; (5) Lưu workflow & RBAC này thành phụ lục bắt buộc | DONE (GREEN) – Đã tạo Agent/Editor roles + policies + 8 permissions (idempotent script 0048_rbac_setup.ts); đã link policy→role trên MySQL qua directus_access (scripts/0048_link_policies_mysql.sql, WHERE NOT EXISTS). PR #110 (CI GREEN: build, Pass Gate, Quality Gate, E2E). Evidence: reports/0048_rbac_execution.md, reports/0048_rbac_merge_result.md, scripts/0048_rbac_setup.ts, scripts/0048_link_policies_mysql.sql |
| 0049-DIFF-UI-STRATEGY | E | Chiến lược hiển thị Diff cho User | Giúp User thấy rõ khác biệt giữa bản cũ/mới trước khi duyệt, tách bạch tri thức vs schema | DONE (GREEN) – Implemented Version History & Diff UI on Nuxt (`knowledge/[id]`). Supports Summary & Content diff via `diff` library + 14 workflow fields. Read-only. Evidence: reports/0049_diff_ui_implementation.md, branch feat/0049-diff-ui. |

Giai đoạn E1 – Content Operations & Agent Workflows

Full requirements: xem [docs/E1_Plan.md](docs/E1_Plan.md)
...
| E1-02-FLOWS-BASIC | E1 | Setup Directus Flows Closed Loop | Trigger async Agent + log Revisions | DONE (GREEN) – Flows created on Directus TEST: `E1: Content Request → Agent Trigger` + `E1: Content Request Audit Log` (idempotent, status=new webhook + audit). Evidence: reports/E1-02_flows_basic_execution.md, PR #120. | DONE |
| E1-03-DASHBOARD-QUEUES | E1 | Dashboard nhắc việc & SLA | Hiển thị “ai giữ bóng”, badge overdue | DONE (GREEN) – Dashboard “Content Operations” + 6 panels (Cần duyệt ngay, Agent đang chạy, Bài mới trong tuần, Tổng bài đang xử lý, SLA: Quá hạn, Hoàn thành tuần này) created on Directus TEST. Evidence: reports/E1-03_dashboards_execution.md, PR #122. | DONE |
| E1-04-UI-NUXT-1TOUCH | E1 | Nuxt Approval Desk 1-chạm (read-only) | UX phê duyệt nhanh, trigger Flow | DONE (GREEN) – Approval Desk `/approval-desk` + detail `/approval-desk/[id]` with requirements/current content visible, actions Approve/Request Changes/Reject wired to content_requests. Evidence: PR #124, reports/E1-04 (UI). | DONE |
| E1-05-FOLDER-TREE | E1 | Folder/Tree display bằng relational | Cấu trúc cha–con `knowledge_documents` | DONE (GREEN) – `/knowledge-tree` with lazy-loaded folders/documents via `knowledge_documents.parent_document_id` + `is_folder`; recursive tree component and composables; no extra tables. Evidence: PR #126, reports/E1-05_folder_tree_execution.md. | DONE |
| E1-06-AGENT-CONNECT-V12 | E1 | Kết nối Agent Data V12 & test E2E | Bắt buộc search trước draft | DONE (GREEN) – Cloud Run `agent-data-test` deployed with image `asia-southeast1-docker.pkg.dev/github-chatgpt-ggcloud/agent-data-test-images/agent-data-test@sha256:f563c5ad9c418feac5a24bb26fa7866581675bd142736ab554262f7817d85cfb`; flows updated to real webhook `https://agent-data-test-pfne2mqwja-as.a.run.app/webhook`; smoke test (scripts/e1-06_smoke_test.ts) created content_request OK. Evidence: E1-02 flow re-run & E1-06 smoke log. | DONE |
| E1-07-ROLE-EXTERNAL | E1 | Map trigger passive External Agents | Rõ ràng correlation_id & IAM | Envelope correlation_id: codex_draft/claude_sinh/antigravity_audit; IAM: system-bot token (draft-only), chữ ký `[Agent_Name]` trong comment/log; trigger 1-click. | TODO |
| E1-08-RBAC-UI | E1 | RBAC UI disable actions vượt quyền | Ẩn/disable Publish/Archive cho Agent | Nuxt guards ẩn nút publish nếu Agent; Editor không sửa schema; hành động nhạy cảm admin-only; test role-based view. | TODO |
