...
const breadcrumb = computed<BreadcrumbItem[]>(() => {
	...
	return [
		{ label: 'Blueprints', slug: '/blueprints', type: 'zone' },
		{ label: document.value.zone, slug: `/blueprints`, type: 'zone' },
...
const metadata = computed(() => ({
	title: document.value?.title || 'Blueprint Document',
	description: document.value?.summary || 'Technical blueprint document',
}));
		...
		<div class="py-8">
			...
			<div v-if="pending" class="flex items-center justify-center py-12">
				<div class="text-center">
					...
					></div>
					<p class="mt-4 text-gray-600 dark:text-gray-400">Loading blueprint...</p>
				</div>
			</div>
			...
			<div v-else-if="error || !document" class="py-12">
				<div class="p-6 text-center border border-red-200 rounded-lg bg-red-50 dark:bg-red-900/20">
					...
					<h3 class="mt-4 text-lg font-semibold text-red-900 dark:text-red-200">Blueprint not found</h3>
					<p class="mt-2 text-red-700 dark:text-red-300">
						The requested blueprint could not be found or is not available.
					</p>
					<NuxtLink
						to="/blueprints"
			...
			<article v-else class="max-w-4xl mx-auto">
				...
				<header class="pb-6 mb-6 border-b border-gray-300 dark:border-gray-700">
					...
					<div class="flex items-start justify-between mb-4">
						<Icon name="heroicons:document-chart-bar" class="w-10 h-10 text-primary-600" />
					...
					<div class="flex flex-wrap gap-2 mt-4">
						<span
							class="px-3 py-1 text-sm font-medium rounded-full bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300"
						>
							<Icon name="heroicons:folder" class="inline-block w-3 h-3 mr-1" />
							{{ document.subZone }}
						</span>
						<span
