...
o	/Users/nmhuyen/Documents/Manual Deploy/agent-data-langroid
- Project Google Cloud Duy nhất:
o	github-chatgpt-ggcloud
- Service Account Duy nhất:
o	chatgpt-deployer@github-chatgpt-ggcloud.iam.gserviceaccount.com
- Các Repository trong Phạm vi:
o	agent-data-test (Repo phát triển & kiểm thử)
o	agent-data-production (Repo sản phẩm)
o	chatgpt-githubnew (Repo trung tâm quản lý secrets)
...
o	14 ngày: Các artifact sẽ được tự động đánh dấu là "stale" (cũ) để cảnh báo sớm.
o	30 ngày: Một quy trình tự động sẽ tạo GitHub Issue [CLEANUP] để yêu cầu phê duyệt dọn dẹp. Việc xóa bỏ chỉ được thực hiện thủ công sau khi Issue được đóng lại.
o	Báo cáo và Cảnh báo: Một báo cáo tự động hàng tuần qua Slack sẽ tổng hợp số lượng artifact đang được đánh dấu "stale". Báo cáo này BẮT BUỘC phải được cấu hình với một ngưỡng cảnh báo (ví dụ: stale_count < 5) và sẽ gửi một cảnh báo đặc biệt nếu số lượng vượt ngưỡng này.
...
o	black, ruff, trufflehog, manifest-drift.
...
o	PR từ nhánh nội bộ: Yêu cầu tất cả các job chính (lint, secret-scan, manifest-drift, terraform-plan, agent-e2e).
o	PR từ fork: Không yêu cầu các job cần secrets/WIF (terraform-plan, agent-e2e).
...
o	Khi một vòng sửa lỗi bắt đầu, log của job thất bại BẮT BUỘC phải được lưu vào một file theo định dạng: .ci/${SHA}.autofix<N>.log (với N là 1 hoặc 2).
o	File log này BẮT BUỘC phải được tải lên (upload) làm artifact của CI run đó.
o	CẤM TUYỆT ĐỐI commit các file log vào repository. Thư mục .ci/ phải được thêm vào file .gitignore.
- Quy trình Tự sửa lỗi (Auto-fix):
o	Vòng 1: Commit bản sửa lỗi với message bắt đầu bằng [AUTOFIX-1] <root-cause>.
o	Vòng 2: Nếu vẫn thất bại, lặp lại quy trình với commit message [AUTOFIX-2] <root-cause>.
o	Sau 2 vòng nếu vẫn thất bại, Cursor phải dừng lại và tạo issue với tiêu đề: 🛑 CI still failing after 2 auto-fixes. Nội dung issue phải đính kèm link đến CI run thất bại cuối cùng.
...
o	Cảnh báo Luân chuyển: Một workflow giám sát (secrets-audit.yml) BẮT BUỘC phải được thiết lập để chạy hàng ngày. Workflow này phải có khả năng gửi cảnh báo qua Slack khi một secret quan trọng còn dưới 15 ngày là đến hạn luân chuyển.

### 7. Quản lý Truy cập (IAM)
- Các quyền được phép cấp cho Service Account (chatgpt-deployer@...):
o	roles/artifactregistry.writer
o	roles/cloudfunctions.developer
o	roles/run.admin
o	roles/secretmanager.secretAccessor
o	roles/storage.admin
o	roles/iam.serviceAccountUser
o	roles/viewer
o	roles/logging.logWriter
o	roles/serviceusage.serviceUsageAdmin
- Các quyền bị cấm tuyệt đối:
o	roles/secretmanager.admin
o	roles/iam.serviceAccountAdmin
o	roles/cloudscheduler.admin
o	roles/pubsub.publisher
