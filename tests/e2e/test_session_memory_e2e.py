...
def test_multi_turn_session_memory():
    ...
    r1 = client.post("/chat", json={"text": "What does the document say about Langroid?"})
    ...
    r2 = client.post(
        "/chat",
        json={"text": "Can you summarize it again in one sentence?", "session_id": sess},
    )
    assert r2.status_code == 200
    body2 = r2.json()
    assert body2.get("session_id") == sess
    assert "langroid" in body2.get("response", "").lower() or "framework" in body2.get("response", "").lower()
