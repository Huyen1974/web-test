...
# Note: This secret was created manually and is now referenced as a data source
# HP-05 Compliance: Terraform references existing secret, doesn't create it
data "google_secret_manager_secret" "directus_admin_password" {
...
resource "google_secret_manager_secret_iam_member" "directus_admin_password_accessor" {
  secret_id = data.google_secret_manager_secret.directus_admin_password.secret_id
  role      = "roles/secretmanager.secretAccessor"
  member    = local.chatgpt_deployer_sa
  project   = var.project_id
}
